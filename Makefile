...
#
# Overrides:  make run PORT=8600     make finetune FT_ROWS=8000 FT_EPOCHS=1
#             make PYTHON=python3 test

PYTHON    ?= python
PORT      ?= 8501
# 0 = use the FULL prepared dataset. This was 8000 while training was
# CPU-bound; with a GPU there is no reason to cap it, and more rows is the
# single biggest lever on the fine-tuned model's accuracy.
# On a CPU-only machine override it:  make finetune FT_ROWS=8000
FT_ROWS   ?= 0
FT_EPOCHS ?= 3
...
train-gpu: ## Fine-tune on the FULL dataset (batch/lr derived from the base model)
	$(PYTHON) finetune/train.py --max-rows 0 --epochs $(FT_EPOCHS) --balanced
	$(PYTHON) finetune/compare.py

retrain: ## Force a fresh fine-tune even if an artifact exists
	$(PYTHON) finetune/train.py --max-rows $(FT_ROWS) --epochs $(FT_EPOCHS) --balanced
	$(PYTHON) finetune/compare.py
