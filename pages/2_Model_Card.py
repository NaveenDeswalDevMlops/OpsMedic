...
"""OpsMedic /model-card — evaluation report for the fine-tuned classifier.

This page answers "can I trust this model?". It is deliberately distinct
from /monitor, which answers "is it healthy right now?": nothing here is
live traffic, everything is read from the artifacts written by
finetune/train.py and finetune/compare.py.

Sections: headline comparison against three baselines, a collapse
detector, per-class precision/recall/F1, the training curve, the exact
hyperparameters used, error analysis, dataset provenance and stated
limitations.
"""
...
from ui import model_card as mc  # noqa: E402
from ui import nav, theme  # noqa: E402

st.set_page_config(page_title="OpsMedic · Model Card", page_icon="🧪",
                   layout="wide")
theme.inject_css()
nav.render_page_chrome("Evaluation", "Model Card")
...
st.caption(
    "Monitor answers 'is it healthy now?'. This page answers 'can I trust "
    "the model?'. Experiments & Cost answers 'which configuration is "
    "better?'."
)
